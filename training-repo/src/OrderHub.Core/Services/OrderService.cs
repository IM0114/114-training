...
public class OrderService : IOrderService
{
    ...
    public async Task<ServiceResult<Order>> CreateOrderAsync(int customerId, IReadOnlyList<NewOrderLine> lines)
    {
        ...
        var validationResult = ValidateOrderLines(lines);
        if (!validationResult.Success)
            return validationResult;

        var errors = new List<string>();
        var order = CreatePendingOrder(customer);

        foreach (var line in lines)
            await TryAddOrderItemAsync(order, line, errors);
    ...
    private static ServiceResult<Order> ValidateOrderLines(IReadOnlyList<NewOrderLine> lines)
    {
        ...
        return ServiceResult<Order>.Ok(null!);
    }

    private static Order CreatePendingOrder(Customer customer) =>
        new()
        {
    ...
    private async Task TryAddOrderItemAsync(Order order, NewOrderLine line, List<string> errors)
    {
